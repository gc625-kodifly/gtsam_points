...
class PGOParams:
    ...
    bidirectional_factors: bool = True
...
def run(
    ...
    *,
    ...
    for node in nodes:
        ...
        shifted[:3, 3] -= offset
        frames.append(gtsam_points_py.FrameData(node.node_id, matrix_to_pose_vec(shifted), node.points))

    trans_sigma = _as_vec3(params.trans_sigma)
    if params.min_trans_sigma > 0.0:
        trans_sigma = [max(float(params.min_trans_sigma), value) for value in trans_sigma]
