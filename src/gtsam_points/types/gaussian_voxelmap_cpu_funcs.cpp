...
merge_frames(const std::vector<Eigen::Isometry3d>& poses, const std::vector<PointCloud::ConstPtr>& frames, double downsample_resolution) {
  ...
  if (has_normals) {
    merged->normals_storage.resize(num_voxels, Eigen::Vector4d::Zero());
    merged->normals = merged->normals_storage.data();
  }
  ...
  for (int i = 0; i < frames.size(); i++) {
    ...
    for (int j = 0; j < frames[i]->size(); j++) {
      ...
      if(has_normals) {
        merged->normals[dest] +=  pose.matrix() * frame::normal(*frames[i], j);
      }
      if (has_intensities) {
